...
import { Search, Bell, Volume2, VolumeX, AlertTriangle, Clock, Package, Check, Play, Sparkles, Cloud, CloudOff, RefreshCw } from 'lucide-react';
import { subscribeToSyncStatus } from '../lib/sync';
...
export default function Header({ onViewChange }: { onViewChange?: (view: string) => void }) {
  ...
  // Background database sync status
  ...
  useEffect(() => {
    const unsubscribe = subscribeToSyncStatus((status, time, err) => {
      ...
      if (err) setSyncErrorMessage(err);
    });
    return () => unsubscribe();
  }, []);
  ...
  return (
    <header id="header" className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-40">
      ...
      <div className="flex items-center gap-6 relative" ref={dropdownRef}>
        {/* Supabase background automatic sync status badge */}
        <button
          onClick={() => onViewChange?.('settings')}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-[11px] font-bold shadow-xs select-none cursor-pointer transition-all hover:scale-102 active:scale-98 ${
            syncStatus === 'synced' ? 'bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-100/75' :
            syncStatus === 'syncing' ? 'bg-indigo-50 text-indigo-700 border-indigo-100 hover:bg-indigo-100/75' :
            syncStatus === 'error' ? 'bg-rose-50 text-rose-700 border-rose-100 hover:bg-rose-100/75' :
            syncStatus === 'not-configured' ? 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100/75' :
            'bg-slate-50 text-slate-600 border-slate-100'
          }`}
          title={
            syncStatus === 'synced' ? `Auto-saving active. Database synced successfully at ${lastSyncTime}.` :
            syncStatus === 'syncing' ? 'Database sync is in progress...' :
            syncStatus === 'error' ? `Database Sync Error: ${syncErrorMessage || 'Unknown error'}. Click to configure/view details.` :
            syncStatus === 'not-configured' ? 'Database Connection: Local-only. Click to configure Supabase Connection!' :
            'Connecting to save engine...'
          }
        >
          {syncStatus === 'synced' && (
            <>
          ...
          {syncStatus === 'error' && (
            <>
              ...
              <span className="hidden sm:inline">Sync Alert</span>
            </>
          )}
          {syncStatus === 'not-configured' && (
            <>
              <Cloud className="w-3.5 h-3.5 text-slate-400" />
              <span className="hidden sm:inline">Local Active</span>
            </>
          )}
          {syncStatus === 'idle' && (
            <>
